...
function TicketSelector({
  ...
  return (
    <div
      className="flex flex-col w-full mx-auto bg-white overflow-hidden"
      style={{
        maxWidth: 412,
...
export default function App() {
  ...
  const handleNextStep = () => {
    if (activeTab === 'entry') goToStep('camping');
    else if (activeTab === 'camping') goToStep('fb');
    else if (activeTab === 'fb') {
      // Final checkout action could go here
      ...
      <main className="pt-[62px] pb-[80px] lg:pb-0 min-h-screen">
          ...
          <div className="max-w-[1280px] mx-auto px-[16px] md:px-[32px] py-[24px]">
            <div className="flex flex-col lg:flex-row gap-[32px]">
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between relative mx-auto max-w-3xl lg:max-w-none">
                  {/* Background line */}
                  <div className="absolute top-[16px] left-[10%] right-[10%] h-[2px] bg-gray-200 -z-10" />
                  
                  {[
                    { id: 'entry', label: '1. Entry ticket', title: 'Entry ticket' },
                    { id: 'camping', label: '2. Camping', title: 'Camping' },
                    { id: 'fb', label: '3. Food & Beverage', title: 'Food & Beverage' }
                  ].map((step, index) => {
                    ...
                    return (
                      <button
                        ...
                        className="flex flex-col items-center gap-[8px] bg-white px-2 cursor-pointer border-none outline-none group"
                      >
                        <div 
                          className="w-[32px] h-[32px] rounded-full flex items-center justify-center text-[14px] font-bold transition-all duration-300"
                          style={{
                        ...
                        <span 
                          className="text-[12px] md:text-[14px] font-semibold transition-colors duration-300"
                          style={{
              ...
              {/* Invisible spacer matching the Ticket Selector width */}
              <div className="hidden lg:block lg:w-[412px] flex-shrink-0" />
            </div>
          </div>
        </div>
        ...
        <div className="max-w-[1280px] mx-auto px-[16px] md:px-[32px] py-[32px]">
          ...
          <div className="flex flex-col lg:flex-row gap-[32px] lg:items-start">
            ...
            <div className="w-full lg:w-[412px] flex-shrink-0 lg:sticky lg:top-[190px] lg:self-start lg:max-h-[calc(100vh-210px)] lg:overflow-y-auto scrollbar-hide">
              ...
              <div className="hidden lg:block">
                ...
                <div className="mt-4">
                  <button
                    onClick={totalPrice === 0 ? undefined : handleNextStep}
                    disabled={totalPrice === 0}
                    className="w-full h-[48px] flex items-center justify-center transition-all hover:opacity-90"
                    style={{
                      background: totalPrice === 0 ? colors.background : colors.primary,
                      borderRadius: '9999px',
                      border: 'none',
                      cursor: totalPrice === 0 ? 'not-allowed' : 'pointer',
                    }}
                  >
                    <span className="text-[16px] font-semibold" style={{ color: totalPrice === 0 ? colors.textMuted : colors.white }}>
                      {totalPrice > 0 
                        ? `${totalPrice.toFixed(2).replace('.', ',')} € — ${activeTab === 'fb' ? 'Comprar ahora' : 'Continuar'}` 
                        : 'Select ticket'}
                    </span>
                  </button>
                </div>
